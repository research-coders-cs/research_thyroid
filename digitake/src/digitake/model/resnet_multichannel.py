...
class Resnet_multichannel(nn.Module):
    def __init__(self, pretrained=True, encoder_depth=50, num_in_channels=4):
        super().__init__()
    ...
    def increase_channels(self, m, num_channels=None, copy_weights=0):
        ...
        :m original model to increase
        :param num_channels to increase, if None, it will increase by 1
        :param copy_weights a channel to copy weight from
        """
        ...
        print("old", m.weight.shape)
        print("new", new_m.weight.shape)

        param = new_m.weight.clone()
        param[:, :m.in_channels, :, :] = m.weight.clone()
        print("new param", param.shape)

        # Copying the weights from the old to the new layer
        # new_m.weight[:, :m.in_channels, :, :] = m.weight.clone()
        # new_m.weight = nn.Parameter(param)

        # Copying the weights of the `copy_weights` channel of the old layer to the extra channels of the new layer
        for i in range(new_in_channels - m.in_channels):
            channel = m.in_channels + i
            param[:, channel:channel + 1, :, :] = m.weight[:, copy_weights:copy_weights + 1, ::].clone()
        new_m.weight = nn.Parameter(param)
